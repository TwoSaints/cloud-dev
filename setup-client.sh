#!/bin/bash
# setup-client.sh — Run on any new Mac to configure SSH access to cloud-dev
...
if [ "$OS" = "mac" ]; then
  ...
  # On Windows, 1Password exposes a named pipe via a compatibility socket
  # Git Bash uses the pageant-compatible pipe at this path
  AGENT_SOCK="$HOME/AppData/Local/1Password/app/8/agent.sock"
  AGENT_SOCK_EXPORT="export SSH_AUTH_SOCK=$HOME/AppData/Local/1Password/app/8/agent.sock"
else
...
# Source the updated shell config for this session
source "$SHELL_RC" 2>/dev/null || true
export SSH_AUTH_SOCK="$AGENT_SOCK"

if ssh -o ConnectTimeout=5 -o BatchMode=yes cloud-dev "echo '  ✓ Connected successfully as $(whoami) on $(hostname)'" 2>/dev/null; then
  echo ""
  echo "=== Setup complete ==="
  echo ""
  echo "You can now:"
